...
public final class AproximativeTokenizer implements Tokenizer {
	public static final AproximativeTokenizer INSTANCE = new AproximativeTokenizer();
	...
	public static final char[] needed = "+=-.,/[]{};:'\"<>\\|?!@#$%^&*()`~".toCharArray();
	...
	public final void tokenize(final String document, final LinkedHashMap<String, LinkedList<Integer>> out) {
		...
		while(cb.hasRemaining()) {
			...
			if(ntype == -2) {
				if(Arrays.binarySearch(needed, c) >= 0 || Character.isWhitespace(c))
					ntype = -1;
